...
if ($article === null) {
    http_response_code(404);
    echo "<h1>Article not found</h1><p><a href='/en/blog.html'>Back to blog</a></p>";
    exit;
}
$translationData = fetchJson(API_BASE . '/articles/' . rawurlencode($slug) . '/translation');
$frSlug = $translationData['slug'] ?? null;
$langSwitchUrl = $frSlug ? "/blog/{$frSlug}" : '/blog.html';

// --- Global fetch ---
$allArticles = fetchJson(API_BASE . '/articles?lang=en') ?? [];
...
function formatDateEn(?string $isoDate): string
{
    if (!$isoDate) return '';
...
$canonicalUrl    = 'https://www.keytchens.com/en/blog/' . rawurlencode($slug);
...
<head>
    ...
    <title><?= e($title) ?> - Keytchens Blog</title>
        ...
        <div class="flex justify-center mt-14 md:mt-14 relative px-4">
            <div class="relative text-center w-full md:w-3/4">
                ...
                <p class="text-lg sm:text-xl text-slate-300 max-w-2xl mx-auto leading-relaxed">
                    The best strategies to grow your restaurant and boost your performance.
                </p>
            </div>
        </div>
    </div>
...
<main class="max-w-screen-xl mx-auto px-4 py-12 lg:py-16">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <article class="lg:col-span-2">
            ...
            <div class="flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-gray-400 mb-8">
                <div class="flex items-center gap-2">
                    ...
                    <time datetime="<?= e($date) ?>"><?= formatDateEn($date) ?></time>
                </div>
            ...
            <div class="flex items-center justify-between mb-8 pb-6 border-b border-gray-100 dark:border-gray-800">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button type="button" class="kc-share-btn kc-share-copy" data-share="copy" title="Copy link">
            ...
            <div class="mt-10">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4">Gallery</h3>
            ...
            <div class="kc-cta-box mt-12 mb-8">
                <div class="relative z-10">
                    <h3 class="text-xl font-bold mb-2">Boost your orders today</h3>
                    <p class="text-emerald-800 dark:text-emerald-200 mb-4 text-sm leading-relaxed">
                        Join the restaurants increasing their revenue on Uber Eats, Deliveroo and Just Eat with Keytchens.
                    </p>
        ...
        <aside class="lg:col-span-1">
            <div class="sticky top-24 space-y-6">
                ...
                <div class="kc-cta-box">
                    <div class="relative z-10">
                        ...
                        <p class="text-emerald-800 dark:text-emerald-200 text-sm mb-4">The ultimate tool to dominate delivery platforms.</p>
                        <a href="/" class="block text-center bg-white text-emerald-700 px-4 py-2 rounded-lg font-semibold text-sm hover:bg-emerald-50 transition">Discover</a>
                    </div>
                </div>

                <!-- Recent articles -->
                <div class="bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 rounded-2xl p-5">
                    <h4 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wide mb-4">Recent articles</h4>
                    <div class="space-y-4">
                       <?php foreach ($recent as $r): ?>
                       <a href="/en/blog/<?= e($r['slug'] ?? '') ?>" class="group flex gap-3 items-start">
            ...
            <time class="text-xs text-gray-400 mt-1 block"><?= formatDateEn($r['date'] ?? null) ?></time>
        </div>
    </a>
    <?php endforeach; ?>
</div>
...
<section class="border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
    <div class="max-w-screen-xl mx-auto px-4 py-12">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <?php if ($prevArticle): ?>
            <a href="/en/blog/<?= e($prevArticle['slug'] ?? '') ?>" class="kc-nav-card group flex items-center gap-4">
                ...
                <div>
                    <span class="text-xs text-gray-400 uppercase tracking-wide">Previous article</span>
            ...
            <?php if ($nextArticle): ?>
            <a href="/en/blog/<?= e($nextArticle['slug'] ?? '') ?>" class="kc-nav-card group flex items-center gap-4 md:text-right md:flex-row-reverse">
                <div class="flex-shrink-0 w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-500 group-hover:bg-emerald-100 group-hover:text-emerald-600 transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                </div>
                <div>
                    <span class="text-xs text-gray-400 uppercase tracking-wide">Next article</span>
...
<section class="bg-white dark:bg-gray-900 py-16 border-t border-gray-100 dark:border-gray-800">
    <div class="max-w-screen-xl mx-auto px-4">
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">You may also like</h2>
            <a href="/en/blog.html" class="text-sm font-semibold text-emerald-600 hover:text-emerald-700 transition">View all articles →</a>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <?php foreach ($related as $a): ?>
            <a href="/en/blog/<?= e($a['slug'] ?? '') ?>" class="group block bg-white dark:bg-gray-800 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <div class="p-5">
                    ...
                    <time class="text-xs text-gray-400 dark:text-gray-500 mt-2 block"><?= formatDateEn($a['date'] ?? null) ?></time>
                </div>
            </a>
            <?php endforeach; ?>
        </div>
    ...
    document.querySelectorAll('.kc-share-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (network === 'linkedin') {
                ...
                copyToClipboard(url)
                    .then(() => showToast('Link copied! Paste it into your Instagram story or bio.'))
...
<script>
    window.KC_LANG_SWITCH_URL = {
        en: <?= json_encode('/en/blog/' . $slug) ?>,
        fr: <?= json_encode($langSwitchUrl) ?>
    };
</script>
</body>
</html>