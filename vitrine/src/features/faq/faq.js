...
function updateResultCount(count) {
  ...
  if (!searchQuery.trim()) {
    ...
    resultCount.textContent = `${count} résultat${count > 1 ? 's' : ''} trouvé${count > 1 ? 's' : ''}`;
  }
}
  ...
  if (container) {
    container.innerHTML = `
      <div class="text-center py-20">
        <p class="text-slate-500">Chargement des questions...</p>
      </div>
    `;
  }

  try {
    const response = await fetch('/src/features/faq/faq-data.json');
    if (!response.ok) throw new Error('Erreur réseau');
    faqData = await response.json();
  } catch (error) {
    console.error('Impossible de charger les données FAQ :', error);
    if (container) {
      container.innerHTML = `
        <div class="text-center py-20">
          <p class="text-slate-500">Une erreur est survenue lors du chargement des questions. Veuillez réessayer plus tard.</p>
        </div>
      `;
    }
    return;
  ...
  if (items.length === 0) {
    container.innerHTML = `
      <div class="text-center py-20 animate-fade-in">
        ...
        <h3 class="text-xl font-semibold text-slate-900 mb-2">Aucun résultat trouvé</h3>
        <p class="text-slate-600 mb-6">Essayez un autre mot-clé ou contactez-nous directement.</p>
